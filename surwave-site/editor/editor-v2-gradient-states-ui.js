(() => {
  ...
  function commonIpSettings(block){
    ensure(block);
    const section=document.createElement('section');section.className='sw-adv-state';
    const head=document.createElement('div');head.className='sw-adv-state-head';head.textContent='Общие настройки IP-карточек';
    const grid=document.createElement('div');grid.className='sw-adv-grid';
    grid.append(label('Непрозрачность чёрного фона',number(block.backgroundOpacity??100,0,100,1,v=>{block.backgroundOpacity=v},'%')));
    const note=document.createElement('div');note.className='sw-adv-note';
    note.textContent='Один параметр для всех состояний. Под полупрозрачным чёрным слоем используется непрозрачный цвет фона сайта, поэтому градиент остаётся только у рамки и не просвечивает под текстом.';
    section.append(head,grid,note);return section;
  }

  function stateEditor(block,layer,name){
    ensure(block);
    const getState=()=>{ensure(block);return block.gradientStates[layer][name]};
    ...
    if(block.type==='servercards'&&layer==='border')grid.append(label('Затемнение блока',number(initial.shade,0,100,1,v=>getState().shade=v,'%')));
    ...
    const note=document.createElement('div');note.className='sw-adv-note';
    if(block.type==='servercards'&&layer==='border')note.textContent='«Затемнение блока» остаётся отдельным направленным слоем. Направление анимации и затемнения следует выбранному режиму.';
  ...
  function layerEditor(block,layer,includeCopied){
    ...
    const body=document.createElement('div');body.className='sw-adv-layer-body';const enabledKey=layer==='border'?'borderGradient':'textGradient';
    body.appendChild(checkbox('Включить',block[enabledKey],v=>block[enabledKey]=v));
    const states=document.createElement('div');states.className='sw-adv-states';['normal','hover','active',...(includeCopied?['copied']:[])].forEach(name=>states.appendChild(stateEditor(block,layer,name)));body.appendChild(states);d.appendChild(body);return d;
  }

  function inject(card,block){
    if(!card||card.querySelector(':scope > .block-body > .sw-advanced-gradient-editor')||!['servercards','linkgroup'].includes(block.type))return;
    ensure(block);const body=card.querySelector(':scope > .block-body');if(!body)return;
    const box=document.createElement('div');box.className='sw-advanced-gradient-editor';
    const title=document.createElement('div');title.className='sw-advanced-title';title.textContent=block.type==='servercards'?'Состояния IP-карточек':'Состояния блока ссылок';
    const help=document.createElement('div');help.className='sw-adv-help';help.textContent=block.type==='servercards'?'Непрозрачность чёрного фона задаётся один раз для всей карточки. Для состояний отдельно настраиваются направление, цвета, затемнение, анимация и скорость.':'До двух цветов. Для каждого состояния отдельно задаются направление, прозрачность цветов, тип анимации и скорость.';
    box.append(title,help);
    if(block.type==='servercards')box.appendChild(commonIpSettings(block));
    box.append(layerEditor(block,'border',block.type==='servercards'),layerEditor(block,'text',block.type==='servercards'));
    body.prepend(box);
  }
  ...
  let queued=false;const queueScan=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;scan()})};
  const observer=new MutationObserver(queueScan);observer.observe(document.body,{childList:true,subtree:true});[0,100,250,500,1000].forEach(ms=>setTimeout(scan,ms));
})();