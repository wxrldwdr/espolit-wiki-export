(() => {
  ...
  function stateDefaults(layer,name){const alpha=layer==='text'?100:({normal:[12,84],hover:[28,94],active:[38,97],copied:[42,98]}[name]||[20,90]);return{mode:'center',color1:'#00ff78',alpha1:Array.isArray(alpha)?alpha[0]:alpha,useSecond:true,color2:'#00ffc0',alpha2:Array.isArray(alpha)?alpha[1]:alpha,animation:'none',speed:2.4,shade:{normal:34,hover:20,active:10,copied:14}[name]??24,backgroundOpacity:100}}
  function normalizeStateInPlace(value,layer,name){
    ...
    if(layer==='border')s.backgroundOpacity=clamp(s.backgroundOpacity,0,100,d.backgroundOpacity);
    return s;
  }
  function migrateLegacy(block){
    ...
    for(const layer of LAYERS){
      ...
      for(const name of STATES){
        ...
        if(!had){target.color1=layer==='border'?legacyBorder1:legacyText1;target.color2=layer==='border'?legacyBorder2:legacyText2;block.gradientStates[layer][name]=target;}
        normalizeStateInPlace(target,layer,name);
      }
    }
    if(block.borderGradient==null)block.borderGradient=block.type==='servercards';if(block.textGradient==null)block.textGradient=block.type==='servercards';return block;
  }
  ...
  function decode(value){try{return JSON.parse(decodeURIComponent(value))}catch(_){return null}}function encode(value){return encodeURIComponent(JSON.stringify(value))}
  ...
  Core.parseDocument=source=>{const metas=collectLinkMeta(source),doc=previousParse(source);let linkIndex=0;walk(doc.blocks,block=>{if(block.type!=='linkgroup')return;const meta=metas[linkIndex++]||null;if(meta?.gradientStates)block.gradientStates=Core.clone(meta.gradientStates);migrateLegacy(block)});return doc};
  ...
  function stateVars(block){migrateLegacy(block);const vars=[];for(const layer of LAYERS){const short=layer==='border'?'b':'t';for(const name of STATES){const key=name==='normal'?'n':name==='hover'?'h':name==='active'?'a':'c',state=block.gradientStates[layer][name];vars.push(`--sw-${short}-${key}-grad:${gradient(state)}`,`--sw-${short}-${key}-anim:${animName(state,layer)}`,`--sw-${short}-${key}-speed:${clamp(state.speed,0.2,60,2.4)}s`);if(layer==='border')vars.push(`--sw-bg-${key}:${clamp(state.backgroundOpacity,0,100,100)/100}`)}}return vars.join(';')}
  function enhanceElement(root,block){if(!root||!block)return;migrateLegacy(block);root.classList.toggle('sw-adv-border',!!block.borderGradient);root.classList.toggle('sw-adv-text',!!block.textGradient);root.dataset.swGradientV='6';stateVars(block).split(';').forEach(pair=>{const at=pair.indexOf(':');if(at>0)root.style.setProperty(pair.slice(0,at),pair.slice(at+1))})}
  function enrichSerialized(output,block){if(!block||!['servercards','linkgroup'].includes(block.type))return output;migrateLegacy(block);const template=document.createElement('template');template.innerHTML=output;const root=block.type==='servercards'?template.content.querySelector('.sw-copy-pair'):template.content.querySelector('.sw-link-gradient');if(!root)return output;enhanceElement(root,block);if(block.type==='linkgroup'){const meta=decode(root.getAttribute('data-sw-link-gradient'))||{};meta.gradientStates=Core.clone(block.gradientStates);root.setAttribute('data-sw-link-gradient',encode(meta))}return template.innerHTML}
  ...
  .sw-copy-pair .sw-copy-card.is-copied{--sw-b-grad:var(--sw-b-c-grad);--sw-b-anim:var(--sw-b-c-anim);--sw-b-speed:var(--sw-b-c-speed);--sw-t-grad:var(--sw-t-c-grad);--sw-t-anim:var(--sw-t-c-anim);--sw-t-speed:var(--sw-t-c-speed);--sw-bg-opacity:var(--sw-bg-c,1)}
  `;Core.previewCss+=CSS;window.SurwaveAdvancedGradientCSS=CSS;window.SurwaveEnsureGradientStates=migrateLegacy;
})();