(() => {
  ...
  function normalizeConfig(config,type){
    ...
    const out={borderGradient:c.borderGradient!=null?!!c.borderGradient:type==='servercards',textGradient:c.textGradient!=null?!!c.textGradient:type==='servercards',gradientStates:{border:{},text:{}}};
  ...
  function ensureStyle(doc){
    ...
    const style=doc.createElement('style');style.id='surwave-gradient-runtime-style';style.textContent=`
      ...
      .sw-copy-card.sw-runtime-border-host::before,.sw-copy-card.sw-runtime-border-host::after,.sw-link-gradient .link-group.sw-runtime-border-host::before{content:none!important;display:none!important;animation:none!important}
      .sw-runtime-border-layer,.sw-runtime-inner-layer,.sw-runtime-shade-layer{position:absolute!important;display:block!important;margin:0!important;padding:0!important;border:0!important;border-radius:inherit!important;pointer-events:none!important}
      ...
      .sw-runtime-inner-layer{z-index:1!important;inset:3px!important;background:#050809!important;transition:inset .12s ease}
      .sw-runtime-shade-layer{z-index:2!important;inset:0!important;background:transparent;transition:background .16s ease}
      .sw-copy-card.sw-runtime-border-host>*:not(.sw-runtime-border-layer):not(.sw-runtime-inner-layer):not(.sw-runtime-shade-layer),.sw-link-gradient .link-group.sw-runtime-border-host>*:not(.sw-runtime-border-layer):not(.sw-runtime-inner-layer):not(.sw-runtime-shade-layer){position:relative!important;z-index:3!important}
  ...
  function animationFrames(kind,isText,state){
    ...
    if(kind==='wave'){
      ...
      const mid=mode==='top'||mode==='bottom'?'50% 50%':'50% 50%';
      return{frames:[{backgroundPosition:dir.start,opacity:.6},{backgroundPosition:mid,opacity:1},{backgroundPosition:dir.end,opacity:.7},{backgroundPosition:dir.start,opacity:.6}],easing:'ease-in-out'};
    }
  ...
  function animateNode(node,state,isText=false){
    ...
    const def=animationFrames(state.animation,isText,state);if(!def||typeof node.animate!=='function')return;
  ...
  function controller(target,config,textElements,type,preview){
    if(target.dataset.swGradientRuntime==='1')return;target.dataset.swGradientRuntime='1';
    const layers=ensureLayers(target);let copiedUntil=0,copiedTimer=null;
    function apply(name){
      ...
      layers.border.hidden=!config.borderGradient;layers.inner.hidden=false;layers.inner.style.setProperty('inset',config.borderGradient?'3px':'0','important');
      layers.inner.style.setProperty('background','#050809','important');
      ...
      if(type==='servercards'){layers.shade.hidden=false;layers.shade.style.backgroundImage=shadeGradient(borderState)}else layers.shade.hidden=true;
      cancelAnimation(target,'_swGlowAnimation');
      if(config.borderGradient&&borderState.animation==='glow'&&typeof target.animate==='function')target._swGlowAnimation=target.animate([{boxShadow:'0 8px 22px rgba(0,0,0,.25),0 0 0 rgba(0,255,120,0)'},{boxShadow:'0 10px 28px rgba(0,0,0,.30),0 0 24px rgba(0,255,192,.28)'},{boxShadow:'0 8px 22px rgba(0,0,0,.25),0 0 0 rgba(0,255,120,0)'}],{duration:borderState.speed*1000,iterations:Infinity,easing:'ease-in-out'});
      applyText(textElements,textState,config.textGradient);
      target.dataset.swGradientState=name;
    }
    function copied(){copiedUntil=Date.now()+5000;target.classList.add('is-copied');const label=target.querySelector('[data-copy-state]');if(label)label.textContent='Скопировано';apply('copied');clearTimeout(copiedTimer);copiedTimer=setTimeout(()=>{copiedUntil=0;target.classList.remove('is-copied');if(label)label.textContent='Нажмите чтобы скопировать';apply(target.matches(':hover')?'hover':'normal')},5000)}
  ...
  window.SurwaveGradientRuntime={init,normalizeConfig,gradient,shadeGradient};
})();