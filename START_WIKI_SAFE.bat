...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_Process ^| Where-Object { ($_.Name -match '^python(w)?\.exe$') -and $_.CommandLine -and ($_.CommandLine -match 'surwave-site[\\/](serve_editor^|serve_migrated)\.py') } ^| ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }" >nul 2>nul
...
echo ============================================================
if "%EXIT_CODE%"=="0" (
  echo Surwave Wiki server stopped.
) else (
  echo Surwave Wiki failed to start.
